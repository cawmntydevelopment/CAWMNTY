import {MetaMaskConnector} from 'wagmi/connectors/metaMask'
import {WalletConnectConnector} from "wagmi/connectors/walletConnect";
import {CoinbaseWalletConnector} from "wagmi/connectors/coinbaseWallet";
import {InjectedConnector} from 'wagmi/connectors/injected'
import {publicProvider} from 'wagmi/providers/public'
import {infuraProvider} from 'wagmi/providers/infura'
import {alchemyProvider} from 'wagmi/providers/alchemy'
import {CHAINS} from "./base";
import {configureChains, Connector, createClient as createWagmiClient} from "wagmi";
import coinbaseWalletIcon from "./assets/coinbaseWalletIcon.svg"
import walletConnectIcon from "./assets/walletConnectIcon.svg"
import {ConnectorInformations} from "./types";
import env from "../../config/env";
import {_} from "../../utils/primary-utils";

const {APP_NAME, INFURA_KEY, ALCHEMY_KEY} = env;

const providers = (() => {
    let _providers = [];

    if (INFURA_KEY && INFURA_KEY.length > 0) {
        _providers.push(infuraProvider({apiKey: INFURA_KEY}));
    }
    if (ALCHEMY_KEY && ALCHEMY_KEY.length > 0) {
        _providers.push(alchemyProvider({apiKey: ALCHEMY_KEY}));
    }

    _providers.push(publicProvider());

    return _providers;
})();

const {chains, provider} = configureChains(CHAINS, providers)

export const metaMaskConnector = new MetaMaskConnector({
    chains,
    options: {
        shimDisconnect: true
    }
});

export const coinbaseWalletConnector = new CoinbaseWalletConnector({
    chains: CHAINS,
    options: {
        appName: APP_NAME as string,
    }
});

export const walletConnectConnector = new WalletConnectConnector({
    chains,
    options: {
        qrcode: true,
    },
});

export const injectedConnector = new InjectedConnector({
    chains,
    options: {
        shimDisconnect: true,
        shimChainChangedDisconnect: true,
        name: (detectedName) =>
            detectedName !== "Injected" ? `Other Injected (${
                typeof detectedName === 'string'
                    ? detectedName
                    : detectedName.join(', ')
            })` : "Injected"
    }
})

export const connectorInformations: ConnectorInformations = {
    metaMask: {
        icon: "data:image/svg+xml;base64,",
    },
    coinbaseWallet: {
        icon: coinbaseWalletIcon,
    },
    walletConnect: {
        icon: walletConnectIcon
    }
}

function filterSelectedConnectors(connectors: Connector[]) {
    let selectedConnectors: Connector[] = [];
    let selectedConnectorKeysRaw: string | string[] = env.CONNECTORS;

    if (typeof selectedConnectorKeysRaw === 'string') {
        selectedConnectorKeysRaw = selectedConnectorKeysRaw.trim()
        selectedConnectorKeysRaw = [selectedConnectorKeysRaw];
    }

    if (selectedConnectorKeysRaw === undefined) {
        selectedConnectors = connectors;
    } else {
        let selectedConnectorKeys: string[] = selectedConnectorKeysRaw.map((key) => key.toLowerCase());
        selectedConnectorKeys = selectedConnectorKeys.map((connectorKey) => connectorKey.trim());

        let unselectedConnectors = [...connectors];
        selectedConnectorKeys.map(selectedConnectorKey => {
            const unselectedConnector = _.remove(unselectedConnectors, (connector: Connector) => {
                let match = connector.id.toLowerCase() === selectedConnectorKey;
                match = match ?? connector.name.toLowerCase() === selectedConnectorKey;
                return match;
            })?.[0];

            !!unselectedConnector && selectedConnectors.push(unselectedConnector);
        });
    }

    const isInjectedAllowed = selectedConnectors.some(connector => connector.id.toLowerCase() === 'injected');
    if (isInjectedAllowed) {
        const isMetamaskAllowed = selectedConnectors.some(connector => connector.id.toLowerCase() === 'metamask');
        const isCoinbaseWalletAllowed = selectedConnectors.some(connector => connector.id.toLowerCase() === 'coinbasewallet');

        const needsInjectedWalletFallback =
            typeof window !== 'undefined' &&
            !!window.ethereum &&
            !(isMetamaskAllowed && window.ethereum.isMetaMask) &&
            !(isCoinbaseWalletAllowed && window.ethereum.isCoinbaseWallet);

        if (!needsInjectedWalletFallback) {
            selectedConnectors = selectedConnectors.filter(connector => connector.id.toLowerCase() !== 'injected');
        }
    }

    return selectedConnectors;
}

const connectors = (() => {
    const metamask = metaMaskConnector;
    const coinbaseWallet = coinbaseWalletConnector;
    const walletConnect = walletConnectConnector;

    const allConnectors = [
        metamask,
        coinbaseWallet,
        walletConnect,
        injectedConnector
    ]

    const selectedConnectors = filterSelectedConnectors(allConnectors);

    return selectedConnectors;
})();

export const wagmiClient = createWagmiClient({
    autoConnect: true,
    connectors,
    provider
});
